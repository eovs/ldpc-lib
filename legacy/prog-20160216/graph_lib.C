...
if (f) f_matr=fopen_or_die("matr.dat", "at", "Problem with opening matr.dat");
for (i=0; i<k; i++)
{ 
	//word2bin(g[i],n,row);
	for (j=0; j<n; j++) 
...
int eq_generations(int n,int r, int g, int *Eqs, int *Nodes)
{
// H is parity-check for base graph
// OUTPUT:
// returns 1 if everything works fine
	   ...
	   for (j=N[L]; j<N[L+1];j++) // over nodes of previous level
	   {
            ...
            for (h=0; h<Numbers[parent]; h++)
			{
				...
				{   //  Good. Add to the list 
                    ...
                    PBranch[nodes]=branch;
					TREES[nodes][0]=j;
					TREES[nodes][1]=sign(child-parent)*branch;
                    DEPTH[nodes]=L+1;
                    // Check for loops
					for(I=0; I<nodes; I++)
					{
...
//										if (Length_new<Length[i]) Length[i]=Length_new;
  ...
  //                                Length[eqs]=Length_new;
   ...
   // new equations found
		...
		{
			TREES[J][0]+=nodesp;        
            tree[nodesp+J][0]=TREES[J][0];        
			tree[nodesp+J][1]=TREES[J][1];        
		}
		for (J=0; J<eqs; J++)
		{
			CYCLESS[eqsp+J][0]+=nodesp;
			CYCLESS[eqsp+J][1]+=nodesp;
		}
		eqsp=eqs;
        nodesp+=nodes;
   }
...
for (J=0; J<eqs; J++)
{
	ACTIVE[CYCLESS[J][0]]=1;
	ACTIVE[CYCLESS[J][1]]=1;
}
...
{
    // mark nodes from tree
	w=0; for (J=0; J<s1; J++) {ACTIVE[TREES[f[J]][0]]=1; 
	//printf("%d : %d ",J, TREE[f[J]][0]); 
	w++;}
    // find again
	s2=0; for (J=0; J<nodes; J++) if (ACTIVE[J]) {f[s2]=J; s2++;}
...
for (J=0; J<eqs; J++)
{
	CYCLESS[J][0]=ACTIVE[CYCLESS[J][0]];     
	CYCLESS[J][1]=ACTIVE[CYCLESS[J][1]];     
}
for (J=0; J<nodes; J++)
	TREES[J][0]=ACTIVE[TREES[J][0]];

for (J=0; J<w; J++)
{
	TREES[J][0]=TREES[f[J]][0];
	TREES[J][1]=TREES[f[J]][1];
}
...
int min_modules(int a[],int eqs, int nodes, int *Mmax)
{
// computes girth-length profile for a given input 
// voltage vector a[1...n]
int i, j, h, new_code=0, w, nd, D[DMAX], wmax=0;
...
for (i=0; i<eqs; i++)
{
	w=abs(WT[CYCLESS[i][0]]-WT[CYCLESS[i][1]]);
	if (w>wmax) wmax=w;
	{
	//	w=abs(w);
		if (w<NDMAX)
		{
			nd=DTABLE[w][0];
			for (h=1; h<=nd; h++) FLAGSS[DTABLE[w][h]]++;
		}
		else
		{
			nd=divisors(w,D); 
			for (h=1; h<=nd; h++) FLAGSS[D[h]]++;
		}
	}
...
int check_eqs(int a[],int eqs, int nodes)
{
// voltage vector a[1...n]
int i, j, J, check=1;
...
for (i=0; i<eqs; i++)
{
    for (J=j; J<=CYCLESS[i][0]; J++)
	{	
		WT[J]=0;
        if (TREES[J][1]!=0)
           WT[J]=WT[TREES[J][0]]+a[abs(TREES[J][1])]*sign(TREES[J][1]);
	}
    j=CYCLESS[i][0]+1;
//    WC[i]=WT[CYCLES[i][0]]-WT[CYCLES[i][1]]; 
    if (WT[CYCLESS[i][0]]==WT[CYCLESS[i][1]]) return -(i+1);  //gfree<g
}
