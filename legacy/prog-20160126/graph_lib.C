...
if (f)	f_matr = fopen_or_die("matr.dat", "at", "matr.dat cannot be opened");
for (i=0; i<k; i++)
{ 
	//word2bin(g[i],n,row);
	for (j=0; j<n; j++) 
...
int randcomb(int n, int w, int pos[])
{
	...
	for (i=0; i<n; i++)
//		if (((n-i)*rand()/RAND_MAX)<w) // what was there initially; kinda works with RAND_MAX = 32767
...
//	err=fopen_s(&f_data, argv[2], "wt" ); fclose(f_data);
	if (1)
	{
		...
		while ((I<N)&& (J<max_codes))
		{
			...
			for (i=1; i<=nt; i++)
			{
				if (am[i]==1 )  a[i]=rand()%M; 	else a[i]=0;
			}
...
int bp_simulation(int b, int c, int M, int maxiter, int Nerr, int Nexp, float SNR, float PR_ERR[]) 
{
    ...
    for (i=0; i<n; i++) 
	    codeword[i]=0;
    i=random_codeword(b, c, M);

    if (i==1) {printf("Bad encoding"); return 0;}
    if (i==-1) {printf("bad matrix "); return i;}

    NSE=NUE=NDE=0; //BSC=0;
    J=0;
    while ((NDE<Nerr) && (J<Nexp)) 
    {
	    if (is_keyboard_hit()) if (get_character()=='x') break;
	    J++;
	    //Noising
	    randnn(noise,n);
	    //Transmitting
	    ...
	    // Decoding
	    iter=bp_decod_lm(ch_out, n,  r, col_weights, row_weights, maxiter);
	    // check result
	    nse=0;
	    for (i=0; i<n; i++)
		    nse+=(ch_out[i]<0)!=codeword[i];
	    if (nse>0) 
	    {
		    // counting errors
		    NSE+=nse; NDE++; if (iter>=0) NUE++;
		    printf("SNR=%5.3f,step=%d,s_ers=%d,f_ers=%d,u_ers=%d,BER=%5.3e,FER=%5.3e\n",SNR, J, NSE, NDE, NUE, (float)NSE/(J+1)/n,(float)NDE/(J+1));
    		if ((NDE>=10)&&((float)NDE/(J+1)>2.5*PR_ERR[1])) break;
	    }
    }

    PR_ERR[0]=(float)NSE/(J+1)/n; //BER
    PR_ERR[1]=(float)NDE/(J+1);  //FER
    return 1;
}
...
for (source=0; source<r; source++)
{
   ...
   while (2*(L+1)<g)
   {
	   ...
	   for (j=N[L]; j<N[L+1];j++) // over nodes of previous level
	   {
			...
			{
                child=Graph[parent][h];      // child node
				if (child<source) continue;    // TO AVOID IDENTICAL CYCLES
                branch=Branches[parent][h];  // branch parent->child
                // check if not back
                if  (branch!=PBranch[j])
				{   //  Good. Add to the list 
...
// msnd   = max symbol node degree
{
	int i, I, j, jb, h, ii, ij, sum, t, g, nt, rt, gt, 
		...
		max_row_w,  //increments
		a[CMAX], besta[CMAX], dd[CMAX], 
	...
	for (i=0; i<DD[2]; i++) //column number
	{
	//	col_weight[i]=2;
		j=i%r;           //row number
		T[j][j]=1; 
	...
	max_row_w=2;
	// random columns of higher weight
	girth[DD[2]]=ace[DD[2]]=0; 
	for (i=0; i<=n;i++) num_cycles[i]=1000000;
	for (t=0; t<trials[0] && num_cycles[n] > 1; t++)
	{
		max_row_w=2;
		...
		for (h=DD[2]; h<n; h++)
		{
			...
			jb=0;
			for (i=0; i<r; i++) 
			if (row_weight[i]<max_row_w) { b[jb]=i; jb++;};
			N2=N3=100000;
	       ...
	       if (score[0]<=num_cycles[h])
			 {
				...
				if ((h==n-1)&&(num_cycles[h]<num_cycles[n]))
				{
				 printf("t=%d, h=%d, girth=%d, ace=%d, num=%d, T2=%d, T3=%d, %d, %d \n", 
					 t,h, girth[DD[2]], ace[h], num_cycles[h], N2, N3, AT2, AT3);
				 if (girth[DD[2]]>6)
					 ii=0;
...
for (i=0; i<n; i++)
{
	dd[i]=0; for (j=0; j<r; j++) dd[i]+=HM[j][i];
}
*/
...
for (i=0; i<r; i++)
{
	if (HM[i][r-1]>0)
	{
		if ((j==0)||(j==2)) HM[i][r-1]=2;
		if (j==1) HM[i][r-1]=1;
		if (j>2) HM[i][r-1]=0;
		j++;
	}
}
/*  // Extra zeros to improve H0
for (t=r; t<n; t++)
{
	if (dd[t]>3)
	{
...
{ 
  ...
  // maxiter is maximum number of iterations	
	...
	while (iter < maxiter)
	{
		...
		{
			synd=0;
			for (j=0; j<rweight[i]; j++) synd^=(soft[V[i][j]]<0);
			if (synd) break;
		}
