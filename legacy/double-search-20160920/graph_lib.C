...
/*
	printf("in=");
    for (i=2000; i<2030; i++) printf("%d ",codeword[i]);
    printf("\n");
*/
...
//j=0; for (i=0; i<n; i++) {codeword[i]^=hard[i]; j+=codeword[i];}
//i=syndrome(row_weights, b, c, M);
//printf(" hard syndrome=%d w=%d \n",i,j);
...
/*************************************************/
/*************************************************/
int rand_base_matrix_g(int locs, int loc[], int num_restr, int rw[], int rmin[], int rmax[], int r0, int r, int n, int msnd, 
	                   int DD[], int trials[], int score[])
// generates random base LDPC matrix HB with degree distribution DD
// upper triangular (dual diagonal)  parity-check part
// DD(i) =  number of columns of weight i
// msnd   = max symbol node degree
{   //FILE *flog;
	int i, I, j, jb, h, ii, ij, sum, t, g, nt, rt, gt, 
		N2, N3,  T2,T3, AT2, AT3,   // overall best
		w, row_weight[CMAX],row_weight0[CMAX], //col_weight[CMAX], 
		max_row_w0, max_row_w,  //increments
		a[CMAX], besta[CMAX], dd[CMAX], b[CMAX],
		girth[CMAX], ace[CMAX], num_cycles[CMAX];
	    // int  DDT[RMAX];
	...
	if ( (DD[rw[i]]<rmin[i]) || (DD[rw[i]]>rmax[i])) return -2;

	// vector of symbol node weights
	
	// non-random (parity-check) part
	for (i=0; i<r-1; i++) dd[i]=2; 
	dd[r0-1]=3; dd[r-1]=3;
	for (i=0; i<locs; i++) dd[r-1-i]++;
   ...
   i=(r0-1)/2; T[i][r0-1]=1; V[i][2]=r0-1; row_weight0[i]=3; 
   i=(r+r0-1)/2; T[i][r-1]=1; V[i][2]=r-1; row_weight0[i]=3; 

   // Additional ones above lower bi-diagonal part
   for (i=0; i<locs; i++)
   {
	   T[loc[i]][r-2-i]=1;
	   V[loc[i]][row_weight0[loc[i]]]=r-2-i;
	   row_weight0[loc[i]]++;
   }
   ...
   {    
		...
		for (h=r; h<n; h++)
		{
			...
			jb=0; for (i=0; i<r; i++) 
				if (row_weight[i]<max_row_w)    // Correction 09092016
				{ b[jb]=i; jb++;}; 
	    	for (I=0; I<trials[1]; I++)
			{
				...
				{
					j=0;
					while (j<w)	j=randcomb(jb, w, a);
					for (ii=0; ii<w; ii++) a[ii]=b[a[ii]];
				}
				else randcomb(r, w, a);
		 // printf("line 1447 h=%d",h); 
	...
	//	      for (ii=0; ii<r; ii++) {for (ij=0; ij<h+1; ij++) 
//		   printf("%d ", T[ii][ij]); printf("\n%");}
//		   for (ii=0; ii<rt; ii++) {for (ij=0; ij<nt; ij++) 
//		   printf("%d ", H[ii][ij]); printf("\n%");}

		    //printf("g=%d, ace=%d, num_cycles=%d  %d %d\n",g, score[1], score[0], girth[h], num_cycles[h]);
		   //flag=0;
...
//		  printf("%d ", T[ii][ij]); printf("\n%");}
				for (i=0; i<w; i++) 
				{
					...
					//printf("h=%d, w=%d, besta[i]=%d max_row_w=%d \n",h,w, besta[i],max_row_w); 
				}
//		 for (ii=0; ii<r; ii++) {for (ij=0; ij<h+1; ij++) 
//		  printf("%d ", T[ii][ij]); printf("\n%");}
		 // for (ii=0; ii<rt; ii++) {for (ij=0; ij<nt; ij++) 
...
// Bi-diagonal s
for (i=0; i<r-1; i++) {HM[i][i]=2; HM[i+1][i]=2;}  HM[r0][r0-1]=0;
// Left-Upper square
HM[0][r0-1]=2; 
HM[(r0-1)/2][r0-1]=3;
// Lower square
...
HM[(r+r0-1)/2][r-1]=3;
HM[r0][r-1]=2;
// Upper right additional locations
for (i=0; i<locs; i++)
{
   HM[loc[i]][r-2-i]=1;
}
// First locations of  columns
for (i=r; i<n; i++)
{
	j=0; while ( (HM[j][i]==0) && (j<r)) j++; 
	if (j==r) 		printf(" weight=0 ");   // program error check
	HM[j][i]=2;
}
  ...
  // maxiter is maximum number of iterations	
	...
	while (iter < maxiter)
	{
	   ...
	   //			{	printf(" iter, check =%d %d ",iter, i);
		...
		//}
		if (!synd){
			// j=0; for (i=0; i<n; i++) j+=hard[i]^codeword[i]; 
			// printf(" synd=0 but errors=%d \n", j);
			return iter;}// no errors detected
